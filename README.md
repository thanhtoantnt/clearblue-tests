...
Bitcode rebuild recipes + recorded results + a bench harness for the **incremental
...
├── bc/                       # per-project bitcode rebuild recipes (no binaries committed)
│   ├── curl/README.md, libuv/README.md, ... (exact flags + PR lists per project)
└── results/                  # local runs only (gitignored; never push)
    ├── curl/summary.tsv
    └── round2/summary.tsv    # libuv / darknet / …
```

No bitcode is committed. Each `bc/<proj>/README.md` documents how to rebuild
**one `old.bc`** (the stored baseline) and **many `pr-<NNNN>.bc`** files — one
bitcode per PR, each built by applying that PR's source onto the baseline and
recompiling with the identical gllvm flags. `run_bench.sh` stores `old.bc`
...
### Rebuild `old.bc` (or per-PR bitcode) from pinned sources

No bitcode is committed; rebuild it first. Upstream trees live in `src/<proj>/`
as git submodules, each pinned to the commit that produced that project's
`old.bc`.
...
```bash
# needs `git submodule update --init` + network to fetch PR refs
...
- **No bitcode is committed** — rebuild it locally from pinned sources
  ([`docs/producing-bitcode.md`](./docs/producing-bitcode.md)). A full sweep of remaining
