# `bc/darknet/` — darknet bitcode for fermat-check incremental-persist
This folder holds typed-pointer LLVM 15 bitcode for **darknet**, used as the input for the `fermat-check` incremental-persist benchmark (`scripts/run_bench.sh`).
## Layout
...
# build + extract bitcode (artifact: darknet binary)
...
get-bc -o pr-NNNN.bc
```
> `scripts/build_pr_bc.sh` automates steps 1-2 for every PR recorded in `results/<proj>/summary.tsv`; run it from the repo root: `./scripts/build_pr_bc.sh darknet`.
...
# 1) store the baseline once (-serialize-seg → ./persist/darknet/<module>/SEG/*.json)
