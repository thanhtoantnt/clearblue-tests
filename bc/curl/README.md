...
From the repo root, run the benchmark harness for this project only. It stores `old.bc` once, then runs each `pr-*.bc` incremental (against the store) and from scratch, writing `results/curl/summary.tsv`.
```bash
export CBC=$HOME/github/FermatAnalyzer/build/tools/cb-check/cb-check
ONLY=curl ./scripts/run_bench.sh
```
Tune with `NWORKERS=8 TIMEOUT=3600 ONLY=curl ./scripts/run_bench.sh`.
...
```bash
./scripts/build_pr_bc.sh curl   # needs upstream source cloned + network
```
